...
def build_data_input_step1(dataprovider):
    ...
    names, lengths, sequences, sequence_sup_data, pssm, targets_step1 = iterator.get_next()
    ...
    targets_step1 = tf.transpose(targets_step1, perm=[1, 0])
...
def build_data_input_step3(dataprovider):
    handle, iterator = dataprovider.get_iterator()
    names, lengths, sequences, sequence_sup_data, pssm, targets_step3 = iterator.get_next()
...
class Model:
    ...
    def build_test_data_input(self):
        ...
        with tf.variable_scope("Input", reuse=None):
            ...
            handle, test_data_dict = build_data_input_step1(test_dataprovider)

            # step1_test_data = (handle, lengths, sequences, sequence_sup_data, structures_step1)
        return handle, test_data_dict, test_dataprovider
    ...
    def build_step1(self, logdir, config=None, test_data=None):
        ...
        with tf.variable_scope("Input", reuse=None):
            if test_data is None:
                ...
                handle, data_dict = build_data_input_step1(step1_dataprovider)
            else:
                is_training = False
                step1_dataprovider = None
                handle, data_dict = test_data
    ...
    def build_step3(self, logdir, config=None, test_data=None):
        ...
        with tf.variable_scope("Input", reuse=None):
            if test_data is None:
                ...
                handle, data_dict = build_data_input_step3(step3_dataprovider)
                is_training = True
            else:
    ...
    def inference(self, step1_logdir, step3_logdir=None):
        ...
        start = time.time()
        handle, test_data_dict, test_dataprovider = self.build_test_data_input()
        # handle, lengths, sequences, sequence_sup_data, structures_step1 = step1_test_data

        model_step1 = self.build_step1(step1_logdir, config=config, test_data=(None, test_data_dict))
        ...
        if step3_logdir is not None:
            ...
            with tf.variable_scope("Input", reuse=None):
                ...
                pssm = tf.placeholder(dtype=tf.float32, shape=[1, None, 20], name="pssm")

                sequences_t = tf.transpose(sequences, perm=[1, 0])
                sequence_sup_data_t = tf.transpose(sequence_sup_data, perm=[1, 0, 2])
                pssm_t = tf.transpose(pssm, perm=[1, 0, 2])

            # step3_test_data = (handle, lengths, sequences_t, sequence_sup_data_t, targets_step3)
            data_dict = {"names": "",
                         "lengths": lengths,
                         "sequences": sequences_t,
                         "sequence_sup_data": sequence_sup_data_t,
                         "pssm": pssm_t,
                         "targets": None}

            model_step3 = self.build_step3(step3_logdir, config=config, test_data=(None, data_dict))
