...
params_to_search_coarse = {"starting_learning_rate": [1.0, 0.1, 0.01, 0.001],
                           ...
                           "train_steps": [100, 200, 500, 1000, 5000],
                           "l2_beta": [1.0, 0.1, 0.01, 0.001, 0.0001, 0.00001]}
...
def do_hyper_params_search():
    ...
    step3_config = joint_model.default_step3_config
    ...
    while has_changed:
        ...
        for key, values in params_to_search_coarse.items():
            ...
            value_before = getattr(step3_config, key)

            for value in values:
                print("Trying %s = %s" % (key, value))
                # step1_config = step1_config._replace(**{key: value})
                step3_config = step3_config._replace(**{key: value})
                # config = config._replace(step1_config=step1_config)
                config = config._replace(step3_config=step3_config)
                ...
                for set_lengths, set_inputs, set_targets, set_predictions, set_step2_predictions, set_step3_predictions in runs:
                    run = zip(set_lengths, set_inputs, set_targets, set_step3_predictions)
                    # decoded_step1_predictions = decoder.decode_step123(run)
                    decoded_step3_predictions = decoder.decode_step123(run)
                    # step1_predictions.append(decoded_step1_predictions)
                    step3_predictions.append(decoded_step3_predictions)

                precisions = []
                recalls = []

                # for predictions in step1_predictions:
                for predictions in step3_predictions:
            ...
            step3_config = step3_config._replace(**{key: best_value})
